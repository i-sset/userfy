...
var _ = Describe("UserRepository", func() {
	Describe("When inserting a valid user", func() {
		...
		BeforeEach(func() {
			validUser = model.User{Firstname: "Josset", Lastname: "Garcia", Email: "isset.josset@gmail.com", Age: 26}
			fakeDB = fakeSQLDB{}
		...
		It("Should call db.Exec with correct insert query", func() {
			expectedQuery := fmt.Sprintf("INSERT INTO users (firstname, lastname, email, age) VALUES (%s, %s, %s, %d)", validUser.Firstname, validUser.Lastname, validUser.Email, validUser.Age)
			userRepository.InsertUser(validUser)
	...
	Describe("When updating an existing user", func() {
		Context("When payload is valid", func() {
			...
			BeforeEach(func() {
				validUser = model.User{ID: 1, Firstname: "Josset", Lastname: "Garcia", Email: "isset.josset@gmail.com", Age: 26}
				fakeDB = fakeSQLDB{}
				userRepository = repository.UserRepository{&fakeDB}
			})
			...
			It("Should call db.Exec method with correct update query", func() {
				expectedQuery := "UPDATE users SET firstname = Josset, lastname = Garcia, email = isset.josset@gmail.com, age = 26  WHERE ID = 1"
				userRepository.UpdateUser(validUser)
